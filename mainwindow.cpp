...
#include <QTextStream>
#include <QDebug>
#define TOLOWER(p) {transform(p.begin(),p.end(),p.begin(),::tolower);}  //转小写
#define ll long long
#define con 1100000
using namespace std;

unsigned char strInput[con], keyInput[con], textChiper[con], descriptionInput[con];
...
//把加密后的数据写入文件
void WriteToTxtFile(const QString &description, const QString &content) {
    QString filePath = "密文.txt";
    QFile file(filePath);

    if (!file.open(QIODevice::Append | QIODevice::Text)) {
        qDebug() << "无法打开文件进行追加:" << file.errorString();
        return;
    }

    // 获取当前时间戳
    QString addTime = QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss");
...
void MainWindow::on_writeToFile_clicked(bool checked)
{
    if (ui->writeToFile->isChecked() && mode == 1) {
        ui -> textEdit_desInput -> setEnabled(true);
    } else {
        ui -> textEdit_desInput -> setEnabled(false);
    }
}


void MainWindow::on_copyToClipboard_clicked(bool checked)
{
...
void MainWindow::on_CatLanModeChange_clicked()
{
    if(miaoMode == 1) {
        miaoMode = 2;
        ui -> CatLanModeChange -> setText("喵语：关");
    } else {
        miaoMode = 1;
        ui -> CatLanModeChange -> setText("喵语：开");
    }
}
...
void MainWindow::on_modeChange_clicked()
{
    if(mode == 1) {
        mode = 2;
        ui -> modeChange ->setText("当前模式：解密");
        ui -> label_input -> setText("在此输入密文");
        ui -> CatLanModeChange -> setEnabled(false);
    } else {
        mode = 1;
        ui -> modeChange ->setText("当前模式：加密");
        ui -> label_input -> setText("在此输入明文");
...
void MainWindow::on_goWork_clicked()
{
    ...
    if(mode==1) {
        ...
        bool fFlag = 0, cFlag = 0;
        if(ui -> writeToFile -> isChecked()) {
            memset(descriptionInput, 0, sizeof descriptionInput);
            QString qdesInput = ui -> textEdit_desInput -> toPlainText();
            WriteToTxtFile(qdesInput, qStrEncrypted);
            fFlag = 1;
        }

        if(ui -> copyToClipboard -> isChecked()) {
            CopyToClipboard(qStrEncrypted);
            cFlag = 1;
        }

        if(fFlag && cFlag) {
            ui -> goWork -> setText("结果已写入文件且复制到剪切板");
        } else if (fFlag && !cFlag) {
            ui -> goWork -> setText("结果已写入文件");
        } else if (!fFlag && cFlag) {
            ui -> goWork -> setText("结果已复制到剪切板");
        } else {
            qDebug() << "6";
        }
        ...
        if(CopyToClipboard(qStrDecrypted)) {
            ui -> goWork -> setText("结果已复制到剪切板中");
        } else {
            ui -> goWork -> setText("处理成功");
        }
    }
}
//完结撒花!!
